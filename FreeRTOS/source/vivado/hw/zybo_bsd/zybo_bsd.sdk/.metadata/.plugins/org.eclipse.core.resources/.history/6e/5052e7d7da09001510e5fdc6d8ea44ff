...
#define AXI_LED_BASE 0x43c30000

static void AXILedBlink(void *parameters)
{
	int i = 0;
	int j = 0;

	while(1) {
		Xil_Out32(XPAR_LEDS_4BITS_BASEADDR, 0x4);

		for(i = 0; i < 5000; i++) {
			for(j = 0; j < 5000; j++);
		}

		Xil_Out32(XPAR_LEDS_4BITS_BASEADDR, 0x8);
...
static void tarea2(void *parameters){
	int btn_state=0, sw_state=0;

	//variables temporales FreeRTOS
	TickType_t LastWakeTime;
	const TickType_t Period = 500; //Se mide en ticks. Definido en FreeRTOSConfig.h

	while(1){
		sw_state=XGpio_DiscreteRead(&sw, 1);
		if (sw_state & 0x1){
			btn_state=XGpio_DiscreteRead(&btn, 1);
			xil_printf("Botones: %d  ---- Switches: %d\n\r",btn_state,sw_state);
		}else{
			xil_printf("Sw1 apagado\n\r");
		}
		//control de tiempo
...
int main( void )
{
	...
	#if( mainSELECTED_APPLICATION == 0 )
	{
			xTaskCreate(AXILedBlink, "AXI_LED", configMINIMAL_STACK_SIZE, NULL, (tskIDLE_PRIORITY + 2), NULL);
			xTaskCreate(tarea2, "btn_sw", configMINIMAL_STACK_SIZE, NULL, (tskIDLE_PRIORITY + 3), NULL);
			vTaskStartScheduler();
	}
	#elif( mainSELECTED_APPLICATION == 1 )
	{
...
XScuGic_Config *pxGICConfig;
	...
	/* Inicializacion de switches y botones*/
